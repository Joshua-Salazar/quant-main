...
class Datalake:
    ...
    def getUploadURL(self, source, user, token, bucket=None, target_folder=None):
        if bucket and target_folder:
            self.url = self.url_base + "source="+source+"&user="+user+"&upload-token=yes&token="+token+"&bucket="+bucket+"&target-folder="+target_folder
        else:
            self.url = self.url_base + "source="+source+"&user="+user+"&upload-token=yes&token="+token
    ...
    def putData(self, source, path, user, token, bucket=None, target_folder=None):
        if  os.path.isfile(path):
            url=self.getUploadURL(source, user, token, bucket, target_folder)
...
#            self.url += '&extra-fields=' + extra_fields + '&X' + extra_fields + '=' + extra_values
        ...
        if ("TICK" in source and ((end-start).total_seconds()) > 0.5*60*60):
