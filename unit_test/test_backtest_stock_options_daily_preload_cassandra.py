...
from ..backtest.strategies.stock_options_daily import StockOptionsDaily
...
from ..backtest.tranche import MonthlyTranche, WeeklyTranche, DailyTranche, RollingTrancheExpiryOffset
...
TEST_CASES = [
    ...
    [datetime(2022, 1, 3), datetime(2023, 3, 10), ['XCBO'], 'RTY Index', 0, 'strike', 0.75, 'rolling', True],
    ...
    [datetime(2022, 1, 2), datetime(2023, 3, 10), ['XCBO'], 'TLT US Equity', 0, 'strike', 0.75, 'rolling', True],
...
class TestStockOptionsDaily(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.test_folder = test_utils.get_test_data_folder("test_stock_options_daily")
    ...
    def test_Single_Underlying(self):
        for item in TEST_CASES:
            ...
            elif tranche_type == 'rolling':
                tranche = RollingTrancheExpiryOffset(start_date, end_date, initial_entry_dates=[start_date, add_business_days(start_date, 5, holidays=get_holidays(calendar, start_date, end_date))], offset=-1, calendar=calendar)
            else:
            ...
            results, nav_series, portfolio_expanded = stock_options_daily(
                ...
                keep_hedges_in_tranche_portfolio=True,
            )
            ...
            portfolio_tranches, pnl_tranches = StockOptionsDaily.format_daily_portfolios_by_legs_and_tranches(results)

            target_file = os.path.join(self.test_folder, f"{test_name}_portfolio_tranches.csv")
            test_utils.assert_dataframe(portfolio_tranches, target_file, self.rebase, ignore_cols=[])

            target_file = os.path.join(self.test_folder, f"{test_name}_pnl_tranches.csv")
            test_utils.assert_dataframe(pnl_tranches, target_file, self.rebase, ignore_cols=[])
